...
export default function ChangeEmailAndUserName({setAuthenticatedUser,tempAuthenticatedUser,updateEmail,setShowModal,loading}){
    const [newEmail,setNewEmail] = useState(tempAuthenticatedUser.universityEmail);
    const [username,setUsername] = useState(tempAuthenticatedUser.username)
    return (
        ...
        <div className="deleteModal max-w-[100%] sm:w-full sm:max-w-[450px] fixed sm:top-[50%] sm:translate-y-[-50%] bg-white rounded-md px-3 py-5">
            ...
            <p className="text-[15px] text-gray-500 mt-2 mb-2">New Username</p>
            <input type="text" className="border border-gray-300 w-full px-3 py-2 rounded-lg" name="usernameInput" placeholder="Enter New Username..." value={username} onChange={(e) => { setUsername(e.target.value) }}/>
            ...
            <div className="options flex justify-end gap-2 mt-4">
                {!loading ? (
                <>
                <button className="bg-black py-1.5 px-2 rounded-md text-white" onClick={(()=>{updateEmail(newEmail,username)})}>Change Email</button>
